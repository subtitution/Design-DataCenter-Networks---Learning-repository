...
1. Подготовьте файлы проектаДобавьте все текущие файлы папки ai_project в индекс Git:bashgit add .
Используйте код с осторожностью.2. Сделайте первый коммитСоздайте базовую точку отсчета для вашего проекта:bashgit commit -m "Initial commit"
Используйте код с осторожностью.(Если Git напишет, что нечего коммитить, значит, ваша папка ai_project пуста. Просто создайте в ней любой файл, например, через команду touch README.md, а затем повторите git add . и git commit).3. Создайте тег зановоТеперь, когда коммит существует, команда сработает без ошибок:bashgit tag -a v1.0.0 -m "The first stable local version 1.0"
Используйте код с осторожностью.4. Отправьте всё на GitHubЗагрузите ваш код вместе с созданным тегом на сайт:bashgit push -u origin main --tags
