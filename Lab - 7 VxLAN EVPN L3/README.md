...
И так картинку выше (это часть картинки из общего вывода с leaf1) я понимаю следующем образом, до mac адреса хоста (*68:05), который живет за leaf3, существует 2-ва маршрута, через AS 65500 и конечная точка AS65503, но понять через какой Spine пойдет трафик из данной команды не получится, и вообще пока не понятно можно ли это узнать не залезая в FIB. <br><br>
