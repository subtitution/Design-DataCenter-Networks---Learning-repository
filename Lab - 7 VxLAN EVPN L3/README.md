...
```
# Разбор Wireshark
## Разбирать будем  описанный выше пример пинга с хоста pc1-1 ping-уем хост pc2-2, который находится за leaf2
И так с Pc1-1(192.168.112.2) выполняем ping 192.168.113.3 (pc2-2).
Трейс снимался на leaf1 (eth1)
И так перед самой отправкой ARP, мы видим как реализуется механизм ARP suppression. Как вы увидите ниже, ретрансляция ответов ARP в VXLAN, реализуется через MP-BGP EVPN (RFC 7432/8365), за счет этого, происходит  оптимизация производительности сети, подавляя широковещательные ARP-запросы и ответ происходит __непосредственно от VTEP__. Это предотвращает широковещательную рассылку по сети VXLAN, повышая масштабируемость и снижая задержку.
__VTEP-серверы изучают__ сопоставление __MAC/IP-адресов удаленных хостов через маршрут BGP EVPN типа 2__ (объявление MAC/IP). Когда поступает ARP-запрос к удаленному хосту, локальный VTEP-сервер перехватывает его и отвечает напрямую, используя полученную информацию, выступая в качестве ARP-прокси.
<img width="868" height="2888" alt="image" src="https://github.com/user-attachments/assets/2403c2f0-97e9-4490-9ff6-3a4ed752839b" />
...
И так вот этим широковещательным запросом, __Кто-ТЫ такой 192.168.113.2???__ достучались до нашего испытуемого подопечного, естественно он безприкословно ответил, даже не подозревая сколько сверху еще добавиться заголовков, в формате описанном в rfc9135 секция 7.1, картинка ниже:<br>
...
И так Пинги успешно прошли, между PC1-1 и PC2-2, предлагаю посмотреть, что поменялось в трейсах и в выводе команд: <br>
## leaf2
```
 vxlan vni 666
VNI to VLAN Mapping for Vxlan1
...
666       4094       vrf1       evpn
```
Как мы видим особо вроде ничего не поменялось, только исчезли Vlan 112 Vlan 113, на соответсвующих Leaf-ах. <br>
## Что поменялось в трейсах?
<img width="1158" height="426" alt="image" src="https://github.com/user-attachments/assets/2b66885d-b943-4f20-a62f-24eedab1ac7f" /> <br>
...
<br>
<br>
На этом пожалуй все, обновленные конфиги leaf1 и 2, в конец данной статьи. А как сделать симметричный routing если на leaf-ах есть одинаковые Vlan-ы?, смотрите в продолжении в следующих постах...) <br>
